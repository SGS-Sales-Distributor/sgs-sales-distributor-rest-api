...
{   
    ...
    {
        $reqData = $request->all();
        $dailyCallPlanData=json_decode(json_encode($reqData['data_toko']));
        $dcPlanDate=json_decode(json_encode($reqData['data_tgl']));
        ...
        try {
            ...
            //     'month_plan' => $request->data['month_plan'],
            //     'year_plan' => $request->data['year_plan'],
            //     'user_id' => $request->data['user_id'],
            // ]);

            $masterCallPlan = new MasterCallPlan;
            $masterCallPlan->month_plan=$request->data['month_plan'];
            $masterCallPlan->year_plan=$request->data['year_plan'];
            $masterCallPlan->user_id=$request->data['user_id'];
            $masterCallPlan->save();

            $dataCallPlanDetail=[];
            foreach($dailyCallPlanData as $index => $data){
                $dataCallPlanDetail[]=[
                    'call_plan_id' => $masterCallPlan->id,
                    'store_id' => $data->code,
                    'date' => $dcPlanDate[$index],
                    // 'created_by' => 
                ];
            };

            if(count($dataCallPlanDetail)>0) MasterCallPlanDetail::insert($dataCallPlanDetail);

            // MasterCallPlanDetail::create([
            //     // 'call_plan_id' => $request->call_plan_id,
            //     'call_plan_id' => $masterCallPlan->id,
            //     'store_id' => $request->data_toko,
            //     'date' => $request->data_tgl,
            // ]);
    ...
    public function updateOneData(Request $request, int $id): JsonResponse
    {
        ...
        try {
            ...
            if(count($dataCallPlanDetail)>0) MasterCallPlanDetail::update($dataCallPlanDetail);
