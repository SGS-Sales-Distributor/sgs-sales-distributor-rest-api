...
class StoreRepository extends Repository implements StoreInterface
{
    ...
    public function getAllData(Request $request): JsonResponse
    {
        ...
        $stores = DB::table('store_info_distri')
            ...
            ->join('master_call_plan_detail', 'master_call_plan_detail.store_id', '=', 'store_info_distri.store_id')
            ->join('master_call_plan', function ($join) use ($userId) {
                $join->on('master_call_plan.id', '=', 'master_call_plan_detail.call_plan_id')
                    ->where('master_call_plan.user_id', '=', $userId);
            })
            ->where('master_call_plan_detail.date', '=', Carbon::now()->format('Y-m-d'))
            ->leftJoin('profil_visit', function ($leftJoin) use ($userId) {
            ...
            ->groupBy([
                ...
                'store_info_distri.store_alias',
                'store_info_distri.store_address',
                'store_info_distri.store_phone',
                'store_info_distri.store_fax',
                'store_info_distri.store_type_id',
                'store_info_distri.subcabang_id',
                'store_info_distri.store_code',
                'store_info_distri.active',
                'profil_visit.id',
                'profil_visit.user',
    ...
    public function storeOneData(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'store_phone' => ['nullable', 'string', 'max:20', 'unique:store_info_distri,store_phone'],
            'store_fax' => ['nullable', 'string', 'max:20', 'unique:store_info_distri,store_fax'],
        ...
        try {
            ...
            $store = StoreInfoDistri::create([
                ...
                'store_phone'     => $request->filled('store_phone') ? $request->store_phone : null,
                'store_fax'       => $request->filled('store_fax') ? $request->store_fax : null,
            ...
            StoreInfoDistriPerson::create([
                ...
                'owner'       => $request->owner,
                'nik_owner'   => $request->filled('nik_owner') ? $request->nik_owner : null,
    ...
    public function updateOneData(Request $request, int $id): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'nik_owner' => 'nullable|string|max:20',
            'email_owner' => 'nullable|string|email',
        ...
        try {
            ...
            $store->update([
                ...
                'updated_by' => $request->userFullname ?? $request->updated_by
            ]);
    ...
    public function storeOwnersData(Request $request, int $id): JsonResponse
    {
        ...
        try {
            ...
            $storeOwner = StoreInfoDistriPerson::create([
                ...
                'created_by' => $request->userFullname ?? $request->created_by,
                'updated_by' => $request->userFullname ?? $request->updated_by,
            ]);
    ...
    //     $cabangUser = DB::table('user_info_cabang')
    //         ->where('user_id', $userId)
    //         ->pluck('cabang_id');
    ...
    //         ->where('store_info_distri.customer_code', '=', DB::raw("(SELECT customer_code FROM user_info WHERE user_id = {$userId})"))
    //         ->where('user_info.user_id', '=', $userId)
    ...
    public function getStoresByUsers(Request $request, $userId): JsonResponse
    {
        $storeByUser = DB::table('store_info_distri')
            ...
            ->leftJoin(
                'store_info_distri_person',
                'store_info_distri_person.store_id',
                '=',
                'store_info_distri.store_id'
            )
